                    ...
                    <div class="row justify-content-end">
                        ...
                        <div class="col-sm-12 col-lg-2 mt-2 mt-lg-0">
                            <button type="button" id="btnEdit" class="btn btn-block btn-danger" disabled>
                                <i class="fas fa-pencil-alt mr-2"></i>Supply
                            </button>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    function changeStatus(data,status,question,commit,successResponse,failedResponse,table) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '{{ url('dashboard/transaksi/purchaseorder') }}/'+status,
                        method: 'post',
                        data: data,
        ...
        $(document).ready(function () {
            let listTable = new Tabulator("#listTable", {
                ...
                pagination: "remote",
                ajaxFiltering: true,
                ajaxURL: "{{ url('dashboard/transaksi/purchaseorder/data') }}",
                ajaxConfig: {
                    method: "POST",
                    headers: {
            ...
            btnEdit.click(function (e) {
                e.preventDefault();
                let id = listTable.getSelectedData()[0].no_po;
                window.location = '{{ url('dashboard/transaksi/receiving/supply') }}/'+id;
            });
